...
import { ArrowRight, Layers, ClipboardList, TrendingUp, ClipboardCheck, Sparkles, Target, RotateCcw } from "lucide-react";
...
export default function HomePage() {
  const { auth, lastAttempt, mistakeInsight, subject, stream, topicSlug, memory, hasMemory } = useLazyTopper();
  const navigate = useNavigate();

  // Returning-user strip if memory exists and user is now logged-in OR even logged-out (subtle).
  const memTopic = memory?.topicSlug ? topicBySlug(memory.topicSlug)?.name : null;
  const memoryStrip = hasMemory && memory && (
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-sm">
          ...
          <span className="text-foreground/85">
            ...
            {memTopic ? ` • ${memTopic}` : ""}
            {memory.mistakeAwareEnabled ? " • Mistake-aware" : ""}
          </span>
        </div>
      </div>
        ...
        <section className="lt-card-elevated p-6 grid md:grid-cols-12 gap-5 items-center bg-gradient-to-br from-card to-secondary/40">
          <div className="md:col-span-5">
            ...
            <div className="text-sm text-muted-foreground">
              {lastAttempt.score}/{lastAttempt.outOf} · {lastAttempt.mode.replace(/-/g, " ")}
            </div>
          </div>
              ...
              <>
                ...
                <div className="flex flex-wrap gap-2 mt-3">
                  <Button asChild size="sm">
                    <Link to={buildPracticePath({ scope: "topic", subject: mistakeInsight.subject, stream: mistakeInsight.stream, topic: mistakeInsight.topicSlug, mode: mistakeInsight.recommendedMode, source: "home", mistake: mistakeInsight.id })}>
                  ...
                  <Button asChild size="sm" variant="secondary">
                    <Link to={buildWorksheetPath({ scope: "topic", subject: mistakeInsight.subject, stream: mistakeInsight.stream, topic: mistakeInsight.topicSlug, mistakeAware: true, source: "home" })}>
                      <ClipboardList className="h-3.5 w-3.5" /> Build from mistakes
                    </Link>
                  </Button>
