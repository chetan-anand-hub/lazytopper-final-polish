    ...
    <div className="space-y-5">
      ...
      {!hasAttempts ? (
        <section className="lt-card p-6 space-y-4">
          ...
          <div className="text-[11px] text-muted-foreground">Sample preview disabled — your real data only.</div>
        </section>
      ) : (
        <>
          {/* KPI strip */}
            ...
            <section className="lt-card-elevated p-5 space-y-3 border-accent/40">
              ...
              <div className="flex flex-wrap gap-2 pt-1 border-t border-border">
                <Button asChild size="sm">
                  <Link to={buildPracticePath({ scope: "topic", subject: mistakeInsight.subject, stream: mistakeInsight.stream, topic: mistakeInsight.topicSlug, mode: mistakeInsight.recommendedMode, source: "me", mistake: mistakeInsight.id })}>
                ...
                return (
                  <article key={a.id} className="rounded-lg border border-border p-3 flex items-center justify-between gap-3">
                    <div className="min-w-0">
                      ...
                      <div className="text-[11px] text-muted-foreground">{a.mode.replace(/-/g, " ")} · {new Date(a.date).toLocaleDateString()}</div>
                    </div>
      ...
      {/* Always show last attempt summary if exists */}
