...
import { Layers, ClipboardList, Sparkles, Timer, FileText, ScrollText, GraduationCap, Lock, Target } from "lucide-react";
...
export default function PracticePage() {
  ...
  const {
    subject, stream, paperScope, topicSlug, selectedTopicSlugs, mistakeAwareEnabled, mistakeInsight, auth,
    setSubject, setStream, setPaperScope, setTopicSlug, setSelectedTopicSlugs, setMode, setMistakeAwareEnabled, setActionSource,
  } = useLazyTopper();
  ...
  useEffect(() => {
    ...
    if (qMistake) setMistakeAwareEnabled(true);
  ...
  const buildRedirect = (m: Mode) => {
    ...
    p.set("mode", m);
    if (mistakeAwareEnabled && mistakeInsight) p.set("mistake", mistakeInsight.id);
  ...
  const previewLine = (m: Mode) => {
    if (m === "full-mock") return `Full ${subject} mock · 80 marks · Sections A–E · 3 hrs${mistakeAwareEnabled && mistakeInsight ? ` · + weak-area mini-section: ${mistakeInsight.topicName}` : ""}`;
  ...
  const PrimaryCard = ({ icon: Icon, title, desc, mode, cta }: { icon: any; title: string; desc: string; mode: Mode; cta: string }) => (
  ...
  return (
    <div className="space-y-5">
      <ContextBar
        ...
        compact
        right={
          <div className="flex items-center gap-2 lt-card px-3 py-1.5">
            <Switch
              id="mistake-aware"
              checked={mistakeAwareEnabled}
              onCheckedChange={(v) => {
                if (auth === "logged-out") {
                  navigate(buildLoginPath({ reason: "mistake-aware", redirect: "/app/practice" }));
                  return;
                }
                if (!mistakeInsight && v) {
                  toast.error("No saved attempts yet. Grade an answer in Check & Improve first.");
                  return;
                }
                setMistakeAwareEnabled(v);
              }}
            />
            <Label htmlFor="mistake-aware" className="text-xs flex items-center gap-1">
              {auth === "logged-out" && <Lock className="h-3 w-3" />}
              Use mistake intelligence
            </Label>
          </div>
        }
      />
      ...
      <div className="grid lg:grid-cols-3 gap-5">
        <div className="lg:col-span-2 space-y-5">
          ...
          <div className="grid sm:grid-cols-2 gap-3">
            ...
            <PrimaryCard icon={ClipboardList} title="Worksheet" desc="Sectioned worksheet, ready for screen or print." mode="worksheet" cta="Open worksheet builder" />
            ...
            <PrimaryCard icon={GraduationCap} title="Full Mock" desc={subject === "Maths" ? "Full Maths mock · 80 marks." : "Full Science mock · 80 marks."} mode="full-mock" cta={`Generate ${subject} full mock`} />
          </div>
            ...
            <AccordionItem value="more" className="border-none">
              ...
              <AccordionContent>
                <div className="grid sm:grid-cols-3 gap-3 pb-2">
                  <PrimaryCard icon={Timer} title="Timed Drill" desc="Short focused drill with timer." mode="timed" cta="Start timed drill" />
          ...
          {/* Paper blueprint when full-mock relevant */}
        ...
        <aside className="space-y-5">
          <MistakeIntelligencePanel source="practice" compact />

          <section className="lt-card p-5 space-y-2">
            <h3 className="font-display text-base font-semibold">Quick links</h3>
            <Link to={buildWorksheetPath({ scope: paperScope, subject, stream, topic: topicSlug ?? undefined, topics: selectedTopicSlugs, source: "practice" })}
              className="text-sm text-primary hover:underline block">→ Open worksheet with same scope</Link>
