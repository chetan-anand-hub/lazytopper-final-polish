...
import { buildLoginPath, buildCheckPath } from "@/lib/navigation";
...
export default function WorksheetPage() {
  ...
  const onSave = () => {
    if (auth === "logged-out") {
      navigate(buildLoginPath({ reason: "save-worksheet", redirect: "/app/practice/worksheet" }));
      return;
    }
        ...
        right={
          <div className="flex items-center gap-2 lt-card px-3 py-1.5">
            <Switch
              ...
              onCheckedChange={(v) => {
                if (auth === "logged-out") {
                  navigate(buildLoginPath({ reason: "mistake-aware-worksheet", redirect: "/app/practice/worksheet" }));
                  return;
                }
            ...
            />
            <Label htmlFor="mistake-aware-ws" className="text-xs flex items-center gap-1">
              {auth === "logged-out" && <Lock className="h-3 w-3" />}
              Build from my mistakes
            </Label>
          </div>
        }
      />
      ...
      <div className="grid lg:grid-cols-3 gap-5">
            ...
            <div className="rounded-lg p-4 bg-accent-soft text-accent-soft-foreground text-sm">
              ...
              <div className="text-xs mt-1.5">
                ...
                {paperScope === "full-subject" || paperScope === "multi-topic"
                  ...
                  : " Worksheet will focus on this exact mistake type."}
              </div>
            </div>
          )}
        ...
        <aside className="space-y-5">
          ...
          <section className="lt-card p-5 text-sm space-y-2">
            ...
            <p className="text-muted-foreground">For full-subject or multi-topic worksheets with mistake-aware on, the weak area becomes a mini-section — it never replaces the whole paper.</p>
          </section>
        </aside>
      </div>
    </div>
