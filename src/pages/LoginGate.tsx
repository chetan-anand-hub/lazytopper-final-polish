...
export default function LoginGate() {
  ...
  const { startTrial, hasMemory } = useLazyTopper();
    ...
    <div className="min-h-screen grid lg:grid-cols-2 bg-background">
      ...
      <section className="flex items-center justify-center p-8">
        <div className="w-full max-w-md space-y-6">
          <div>
            <div className="lt-chip-accent inline-flex"><Sparkles className="h-3 w-3" /> {hasMemory ? "Welcome back" : "Free trial"}</div>
            <h2 className="font-display text-3xl font-semibold mt-3">{hasMemory ? "Log in to continue" : "Sign in / Start trial"}</h2>
          ...
          <form className="space-y-3" onSubmit={(e) => { e.preventDefault(); onTrial(); }}>
            ...
            <Button type="submit" className="w-full">
              {hasMemory ? "Resume LazyTopper" : "Start free trial"} <ArrowRight className="h-4 w-4" />
            </Button>
          </form>
