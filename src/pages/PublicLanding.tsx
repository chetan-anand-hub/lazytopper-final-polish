...
import { ArrowRight, Sparkles, Target, ClipboardCheck, TrendingUp, GraduationCap, BookOpen, Brain, RotateCcw } from "lucide-react";
import { useLazyTopper } from "@/context/LazyTopperContext";
import { topicBySlug } from "@/lib/topics";

export default function PublicLanding() {
  const { hasMemory, memory, clearMemory } = useLazyTopper();
  const navigate = useNavigate();

  const lastTopic = memory?.topicSlug ? topicBySlug(memory.topicSlug)?.name : null;
  const lastFocus = memory
    ? `${memory.subject}${memory.subject === "Science" && memory.stream !== "All" ? ` · ${memory.stream}` : ""}${lastTopic ? ` • ${lastTopic}` : ""}${memory.mistakeAwareEnabled ? " • Mistake-aware" : ""}`
    : null;

  const resumeTo = memory?.lastRoute && memory.lastRoute.startsWith("/app") ? memory.lastRoute : "/app";

  const onResume = () => {
    navigate(`/app/login?reason=login&redirect=${encodeURIComponent(resumeTo)}`);
  };
  const onStartFresh = () => {
    clearMemory();
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      {hasMemory && memory && (
        <section className="border-b border-accent/30 bg-accent-soft/40">
          <div className="max-w-6xl mx-auto px-6 py-4 flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-3">
              <div className="h-8 w-8 rounded-full bg-accent text-accent-foreground grid place-items-center">
                <RotateCcw className="h-4 w-4" />
              </div>
              <div>
                <div className="text-sm font-semibold text-foreground">Welcome back — continue where you left off</div>
                <div className="text-xs text-muted-foreground">Last focus: {lastFocus}</div>
              </div>
            </div>
            <div className="flex flex-wrap gap-2">
              <Button size="sm" onClick={onResume}>
                Resume LazyTopper <ArrowRight className="h-3.5 w-3.5" />
              </Button>
        ...
        <div className="max-w-6xl mx-auto px-6 py-20 md:py-28 grid lg:grid-cols-12 gap-10 items-center">
          <div className="lg:col-span-7">
            ...
            <h1 className="font-display text-5xl md:text-6xl font-semibold leading-[1.05] tracking-tight">
              Your CBSE board <span className="text-accent">cockpit</span>.
              <br />
              Not another planner.
            </h1>
            <p className="mt-5 text-lg text-muted-foreground max-w-xl">
              LazyTopper is intent-first. Pick a topic, generate a worksheet, attempt a full 80-mark mock, or check your own answers.
