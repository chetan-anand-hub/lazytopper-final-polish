...
export function MistakeIntelligencePanel({ source, compact, bare }: MistakeIntelligencePanelProps) {
  ...
  if (auth === "logged-out") {
    const redirect = source === "practice" ? "/app/practice" : source === "worksheet" ? "/app/practice/worksheet" : `/app/${source === "home" ? "" : source}`;
    return (
  ...
  if (!mistakeInsight) {
    return (
      <section className={`lt-card p-5 ${compact ? "" : "space-y-3"}`}>
        ...
        <p className="text-sm text-muted-foreground">
          Grade an answer in <Link to="/app/check" className="text-primary underline-offset-2 hover:underline">Check &amp; Improve</Link> or save an attempt to start seeing your weak areas here.
        </p>
      </section>
    );
  }
    ...
    <section className="lt-card p-5 space-y-4">
      ...
      {!compact && (
        <div className="flex flex-wrap gap-2">
          <Button asChild size="sm">
            <Link to={buildPracticePath({ scope: "topic", subject: mistakeInsight.subject, stream: mistakeInsight.stream, topic: mistakeInsight.topicSlug, mode: mistakeInsight.recommendedMode, source, mistake: mistakeInsight.id })}>
          ...
          <Button asChild size="sm" variant="secondary">
            <Link to={buildWorksheetPath({ scope: "topic", subject: mistakeInsight.subject, stream: mistakeInsight.stream, topic: mistakeInsight.topicSlug, mistakeAware: true, source })}>
              <ClipboardList className="h-3.5 w-3.5" /> Build from this mistake
            </Link>
          </Button>
          <Button asChild size="sm" variant="secondary">
            <Link to={buildPracticePath({ scope: "full-subject", subject: mistakeInsight.subject, stream: mistakeInsight.stream, mode: "full-mock", source, mistake: mistakeInsight.id })}>
              <Layers className="h-3.5 w-3.5" /> Add to next mock
            </Link>
          </Button>
          <Button asChild size="sm" variant="ghost">
            <Link to={buildTopicPath(mistakeInsight.topicSlug, source)}>
              <BookOpen className="h-3.5 w-3.5" /> Open topic
            </Link>
          </Button>
        </div>
      )}
