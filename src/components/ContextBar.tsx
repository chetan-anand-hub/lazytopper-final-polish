...
interface ContextBarProps {
  ...
  /** When true, render compact (smaller heading, no subtitle styling) for in-app workspace pages. */
  compact?: boolean;
}

export function ContextBar({ title, subtitle, right, compact }: ContextBarProps) {
  ...
  return (
    <header className="mb-5 animate-fade-in">
      <div className="flex flex-wrap items-center gap-2 text-xs text-muted-foreground mb-2">
        ...
        <span className="lt-chip">Mode: {modeLabel}</span>
        {mistakeAwareEnabled && (
